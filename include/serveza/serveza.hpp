...
public:
   ...
   *
   * @param ex     The main executor; may be used to dispatch or post additional work.
   ...
   * @param ec     Error code set if an error occurred during coroutine execution with @c yield.
   */
  virtual void operator()(boost::asio::any_io_executor ex, typename protocol_type::socket socket,
                          boost::asio::yield_context yield, const boost::system::error_code& ec) noexcept = 0;
};
...
public:
  ...
  void on_state_changed(lifecycle_callback cb) final
  {
    if (!cb) return;
    namespace net = boost::asio;
    net::dispatch(m_strand, [self = this->shared_from_this(), cb = std::move(cb)]() mutable {
...
protected:
  ...
  {
    boost::asio::defer(m_ex, [ex, where = std::move(where), cbs = m_error_cbs] {
      for (auto& cb : cbs) {
        try {
          cb(ex, where);
        } catch (...) {}
...
protected:
  void start_impl(boost::asio::yield_context yield)
  {
    ...
    try {
      sys::error_code ec;
      std::make_from_tuple<Session>(this->m_args)(this->m_ex, std::move(socket), yield[ec], ec);
      if (ec && ec != net::error::operation_aborted) throw sys::system_error{ec};
    } catch (...) {
...
protected:
  void start_impl(boost::asio::yield_context yield)
  {
    ...
    boost::system::error_code ec;
    acceptor_type acceptor{net::make_strand(this->m_ex), this->m_endpoint};
    this->set_state(lifecycle_state::started);
    ...
    while (yield.cancelled() == net::cancellation_type::none) {
      ...
      if (ec) {
        ...
        this->emit_error(ec, "acceptor::accept()");
        continue;
      }

      auto ex = socket.get_executor();
      auto [key, sig] = emplace_cancellation(ex);
      ...
      net::spawn(
          ...
          [self = this->shared_from_this(), socket = std::move(socket), key = key](net::yield_context yield) mutable {
            try {
              sys::error_code ec;
              std::make_from_tuple<Session>(self->m_args)(self->m_ex, std::move(socket), yield[ec], ec);
              if (ec && ec != net::error::operation_aborted) throw sys::system_error{ec};
            } catch (...) {
              self->emit_error(std::current_exception(), "session::operator()");
            }

            net::defer(self->m_strand, [self, key] { self->m_cancels.erase(key); });
          },
  ...
  void stop_impl()
  {
    ...
    for (auto& [_, cancel] : this->m_cancels)
      net::post(cancel.ex, [&sig = cancel.sig] { sig.emit(net::cancellation_type::all); });
  }

private:
  struct cancelation final {
    explicit cancelation(boost::asio::any_io_executor ex)
      : ex(std::move(ex))
    {}
  ...
  [[nodiscard]]
  std::pair<std::size_t, boost::asio::cancellation_signal&> emplace_cancellation(boost::asio::any_io_executor ex)
  {
    while (true) {
      if (m_cancels_key_seq == std::numeric_limits<std::size_t>::max()) m_cancels_key_seq = 0;
      auto [it, ok] = m_cancels.try_emplace(++m_cancels_key_seq, ex);
      if (ok) return {it->first, it->second.sig};
    }
  }

  std::size_t m_cancels_key_seq{0};
  std::unordered_map<std::size_t, cancelation> m_cancels;
