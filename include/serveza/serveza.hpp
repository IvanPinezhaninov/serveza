...
protected:
  void start_impl(boost::asio::yield_context yield)
  {
    ...
    while (yield.cancelled() == net::cancellation_type::none) {
      ...
      auto [key, sig] = emplace_cancellation();
...
private:
  [[nodiscard]]
  std::pair<std::size_t, boost::asio::cancellation_signal&> emplace_cancellation()
  {
    while (true) {
      if (m_cancels_key_seq == std::numeric_limits<std::size_t>::max()) m_cancels_key_seq = 0;
      auto [it, ok] = m_cancels.try_emplace(++m_cancels_key_seq);
      if (ok) return {it->first, it->second};
    }
  }

  template<typename CompletionToken>
  auto async_erase_cancellation(std::size_t key, CompletionToken&& token)
  {
    namespace net = boost::asio;
    ...
    return net::async_initiate<CompletionToken, void()>(
        [self = this->shared_from_this(), key](auto handler) mutable {
          net::defer(self->m_strand, [self, key, handler = std::move(handler)]() mutable {
            ...
            if (self->m_cancels.empty() && self->get_state() == lifecycle_state::stopping)
              self->set_state(lifecycle_state::stopped);
            handler();
          });
        },
