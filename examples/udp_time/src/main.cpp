...
public:
  void operator()(net::any_io_executor ex, protocol_type::socket socket, net::yield_context yield) override
  {
    ...
    net::steady_timer finishingTimer{yield.get_executor()};
    ...
    while (yield.cancelled() == net::cancellation_type::none) {
      ...
      auto [key, sig] = emplace_cancellation(cancelsKeySeq, cancels);
      net::spawn(
          strand,
          [this, ex, session_ex = yield.get_executor(), &socket, remote, &cancels, &finishingTimer, &finishing,
           key = key](net::yield_context yield) mutable {
            timeBroadcast(ex, socket, remote, yield);
            net::defer(session_ex, [&cancels, &finishingTimer, &finishing, key] {
              cancels.erase(key);
              if (finishing && cancels.empty()) finishingTimer.cancel();
            });
    ...
    if (!cancels.empty()) {
      finishingTimer.expires_at(net::steady_timer::time_point::max());
      finishingTimer.async_wait(yield[ec]);
    }
...
private:
  ...
  emplace_cancellation(std::size_t& sigsKeySeq, std::unordered_map<std::size_t, boost::asio::cancellation_signal>& sigs)
  {
    while (true) {
